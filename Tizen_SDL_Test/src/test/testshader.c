...
static SDL_bool CompileShader(GLhandleARB shader, const char *source)
{
    ...
    if (status == 0) {
        ...
        SDLTest_LogError(SDL_LOG_CATEGORY_APPLICATION, "Failed to compile shader:\n%s\n%s", source, info);
        SDL_stack_free(info);
...
static SDL_bool InitShaders()
{
    ...
    for (i = 0; i < NUM_SHADERS; ++i) {
        if (!CompileShaderProgram(&shaders[i])) {
            SDLTest_LogError(SDL_LOG_CATEGORY_APPLICATION, "Unable to compile shader!\n");
            return SDL_FALSE;
        }
    }
...
int main(int argc, char **argv)
{
    ...
    if ( SDL_Init(SDL_INIT_VIDEO) < 0 ) {
        SDLTest_LogError(SDL_LOG_CATEGORY_APPLICATION, "Unable to initialize SDL: %s\n", SDL_GetError());
        exit(1);
    }
    ...
    if ( !window ) {
        SDLTest_LogError(SDL_LOG_CATEGORY_APPLICATION, "Unable to create OpenGL window: %s\n", SDL_GetError());
        SDL_Quit();
        exit(2);
    }

    if ( !SDL_GL_CreateContext(window)) {
        SDLTest_LogError(SDL_LOG_CATEGORY_APPLICATION, "Unable to create OpenGL context: %s\n", SDL_GetError());
        SDL_Quit();
        exit(2);
    }
    ...
    if ( ! surface ) {
        SDLTest_LogError(SDL_LOG_CATEGORY_APPLICATION, "Unable to load icon.bmp: %s\n", SDL_GetError());
        SDL_Quit();
        exit(3);
    }
    ...
    InitGL(640, 480);
    if (InitShaders()) {
        SDLTest_Log("Shaders supported, press SPACE to cycle them.\n");
    } else {
        SDLTest_LogError(SDL_LOG_CATEGORY_APPLICATION, "Shaders not supported!\n");
    }
    done = 0;
    while ( ! done ) {
...
{
    SDLTest_LogError(SDL_LOG_CATEGORY_APPLICATION, "No OpenGL support on this system\n");
    return 1;
}
